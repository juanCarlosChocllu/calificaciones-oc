...
export class AutenticacionService {
  ...
  async login(createAutenticacionDto: CreateAutenticacionDto) {
     const user = await this.UserService.findOneUser(createAutenticacionDto.user)
     const password= await bcrypt.compare(createAutenticacionDto.password, user.password)
      if(!password){
